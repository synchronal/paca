...
use crate::cache::{blobs_dir, derive_tag, get_hub_dir, model_dir_name, read_ref, snapshots_dir};
...
impl FromStr for RemoveTarget {
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        if s.contains(':') {
            let model_ref: ModelRef = s.parse()?;
            return Ok(Self::Tag(model_ref));
        }
...
pub fn remove_model(target: &str, hub_dir: Option<PathBuf>) -> Result<RemoveResult, PacaError> {
    ...
    match target {
        RemoveTarget::Repo { owner, model } => remove_repo(&hub_dir, &owner, &model),
        RemoveTarget::Tag(model_ref) => remove_tag(&hub_dir, &model_ref),
    }
}
...
fn remove_tag(hub_dir: &Path, model_ref: &ModelRef) -> Result<RemoveResult, PacaError> {
    let model_dir = hub_dir.join(model_dir_name(model_ref));
    if !model_dir.is_dir() {
        return Err(PacaError::ModelNotInstalled(model_ref.to_string()));
    }

    let commit = read_ref(hub_dir, model_ref)
        .map(|c| c.trim().to_string())
        .ok_or_else(|| PacaError::ModelNotInstalled(model_ref.to_string()))?;

    let snapshot_dir = snapshots_dir(hub_dir, model_ref).join(&commit);
    ...
    if blobs.is_dir() {
        for blob_entry in fs::read_dir(&blobs)? {
...
/// matching file is returned individually. For a shard-layout tag (gguf files
/// nested under a `{tag}/` subdirectory), the subdirectory itself is returned
/// once so callers can remove the entire shard tree with a single call.
fn find_tag_entries(
    snapshot_dir: &Path,
    model: &str,
    tag: &str,
...
fn collect_referenced_blob_hashes(
    ...
    for entry in fs::read_dir(dir)? {
        ...
        if let Ok(target) = fs::read_link(&path) {
            ...
            if let Some((_, hash)) = target_str.rsplit_once("blobs/") {
...
fn snapshot_contains_gguf(dir: &Path) -> Result<bool, PacaError> {
    for entry in fs::read_dir(dir)? {
        ...
        } else if entry.file_name().to_string_lossy().ends_with(".gguf") {
            return Ok(true);
        }
    }
    Ok(false)
...
mod tests {
    ...
    fn setup_model_dir(hub: &Path, owner: &str, model: &str) -> PathBuf {
    ...
    fn write_blob(model_dir: &Path, hash: &str) {
        fs::write(model_dir.join("blobs").join(hash), b"fake blob data").unwrap();
    }

    fn write_ref(model_dir: &Path, commit: &str) {
        fs::write(model_dir.join("refs").join("main"), commit).unwrap();
    }

    fn write_snapshot_symlink(model_dir: &Path, commit: &str, filename: &str, blob_hash: &str) {
    ...
    fn remove_tag_removes_single_file_symlink_and_orphaned_blob() {
        ...
        // When the only tag is removed, the entire model dir is removed too.
